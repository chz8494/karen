...
<?php if ( bp_is_group_admin_screen( 'group-settings' ) ) : ?>
	...
	<?php if ( bp_is_active( 'forums' ) ) : ?>
		...
		<?php if ( bp_forums_is_installed_correctly() ) : ?>
			...
			<div class="checkbox">
				<label for="group-show-forum"><input type="checkbox" name="group-show-forum" id="group-show-forum" value="1"<?php bp_group_show_forum_setting(); ?> /> <?php esc_html_e( 'Enable discussion forum', 'buddypress' ); ?></label>
			</div>
...
<?php if ( bp_is_group_admin_screen( 'group-avatar' ) ) : ?>
	...
	<?php if ( 'upload-image' == bp_get_avatar_admin_step() ) : ?>
			...
			<?php if ( bp_get_group_has_avatar() ) : ?>
				...
				<?php bp_button( array( 'id' => 'delete_group_avatar', 'component' => 'groups', 'wrapper_id' => 'delete-group-avatar-button', 'link_class' => 'edit', 'link_href' => bp_get_group_avatar_delete_link(), 'link_title' => esc_html__( 'Delete Group Profile Photo', 'buddypress' ), 'link_text' => esc_html__( 'Delete Group Profile Photo', 'buddypress' ) ) ); ?>
...
<?php if ( bp_is_group_admin_screen( 'group-cover-image' ) ) : ?>
	...
	<h4><?php esc_html_e( 'Change Cover Image', 'buddypress' ); ?></h4>
...
<?php if ( bp_is_group_admin_screen( 'manage-members' ) ) : ?>
	...
	<div class="bp-widget">
		<h4><?php esc_html_e( 'Administrators', 'buddypress' ); ?></h4>
		...
		<ul id="admins-list" class="item-list single-line">
			...
			<li>
				<?php echo bp_core_fetch_avatar( array( 'item_id' => bp_get_member_user_id(), 'type' => 'thumb', 'width' => 30, 'height' => 30, 'alt' => sprintf( esc_html__( 'Profile picture of %s', 'buddypress' ), bp_get_member_name() ) ) ); ?>
				<h5>
	...
	<?php if ( bp_group_has_moderators() ) : ?>
		<div class="bp-widget">
			<h4><?php esc_html_e( 'Moderators', 'buddypress' ); ?></h4>
				...
				<ul id="mods-list" class="item-list single-line">
					...
					<li>
						...
						<h5>
							<a href="<?php bp_member_permalink(); ?>"> <?php bp_member_name(); ?></a>
							<span class="small">
								<a href="<?php bp_group_member_promote_admin_link( array( 'user_id' => bp_get_member_user_id() ) ); ?>" class="button confirm mod-promote-to-admin" title="<?php esc_attr_e( 'Promote to Admin', 'buddypress' ); ?>"><?php esc_html_e( 'Promote to Admin', 'buddypress' ); ?></a>
					...
					<li class="<?php bp_group_member_css_class(); ?>">
						...
						<h5>
							...
							<?php else : ?>
								...
								<a href="<?php bp_group_member_ban_link(); ?>" class="button confirm member-ban" title="<?php esc_attr_e( 'Kick and ban this member', 'buddypress' ); ?>"><?php esc_html_e( 'Kick &amp; Ban', 'buddypress' ); ?></a>
								<a href="<?php bp_group_member_promote_mod_link(); ?>" class="button confirm member-promote-to-mod" title="<?php esc_attr_e( 'Promote to Mod', 'buddypress' ); ?>"><?php esc_html_e( 'Promote to Mod', 'buddypress' ); ?></a>
								<a href="<?php bp_group_member_promote_admin_link(); ?>" class="button confirm member-promote-to-admin" title="<?php esc_attr_e( 'Promote to Admin', 'buddypress' ); ?>"><?php esc_html_e( 'Promote to Admin', 'buddypress' ); ?></a>
