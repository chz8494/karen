...
get_header( 'buddypress' ); ?>
	...
	<div class="page-title-bold"><?php esc_html_e( 'Create a Group', 'buddypress' ); ?></div>
		...
		<form action="<?php bp_group_creation_form_action(); ?>" method="post" id="create-group-form" class="standard-form" enctype="multipart/form-data">
			<a class="color-button" href="<?php echo trailingslashit( bp_get_root_domain() . '/' . bp_get_groups_root_slug() ); ?>"><?php esc_html_e( 'Groups Directory', 'cinematix' ); ?></a>
		...
		<div class="item-body" id="group-create-body">
			...
			<?php if ( bp_is_group_creation_step( 'group-settings' ) ) : ?>
				...
				<?php if ( bp_is_active( 'forums' ) ) : ?>
					...
					<h4><?php esc_html_e( 'Group Forums', 'buddypress' ); ?></h4>

					<?php if ( bp_forums_is_installed_correctly() ) : ?>

						<p><?php esc_html_e( 'Should this group have a forum?', 'buddypress' ); ?></p>

						<div class="checkbox">
							<label for="group-show-forum"><input type="checkbox" name="group-show-forum" id="group-show-forum" value="1"<?php checked( bp_get_new_group_enable_forum(), true, true ); ?> /> <?php esc_html_e( 'Enable discussion forum', 'buddypress' ); ?></label>
						</div>
					<?php elseif ( is_super_admin() ) : ?>

						<p><?php
						printf( wp_kses( __( '<strong>Attention Site Admin:</strong> Group forums require the <a href="%s">correct setup and configuration</a> of a bbPress installation.', 'buddypress' ), $allowed_html_array ), bp_core_do_network_admin() ? network_admin_url( 'settings.php?page=bb-forums-setup' ) :  admin_url( 'admin.php?page=bb-forums-setup' ) );
						?></p>
