...
class BBP_Default extends BBP_Theme_Compat {
	...
	private function setup_globals() {
		$bbp           = bbpress();
		$this->id      = 'default';
		$this->name    = esc_html__( 'bbPress Default', 'bbpress' );
		$this->version = bbp_get_version();
		$this->dir     = trailingslashit( $bbp->themes_dir . 'cinematix' );
		$this->url     = trailingslashit( $bbp->themes_url . 'cinematix' );
	}
	...
	public function localize_topic_script() {
		...
		if ( bbp_is_favorites_active() ) {
			...
			$localizations['favLinkYes']      = esc_html__( 'favorites',                                         'bbpress' );
			$localizations['favLinkNo']       = esc_html__( '?',                                                 'bbpress' );
			$localizations['favYes']          = esc_html__( 'This topic is one of your %favLinkYes% [%favDel%]', 'bbpress' );
			$localizations['favNo']           = esc_html__( '%favAdd% (%favLinkNo%)',                            'bbpress' );
			$localizations['favDel']          = esc_html__( '&times;',                                           'bbpress' );
			$localizations['favAdd']          = esc_html__( 'Add this topic to your favorites',                  'bbpress' );
		} else {
		...
		if ( bbp_is_subscriptions_active() ) {
			...
			$localizations['subsSub']      = esc_html__( 'Subscribe',   'bbpress' );
			$localizations['subsUns']      = esc_html__( 'Unsubscribe', 'bbpress' );
